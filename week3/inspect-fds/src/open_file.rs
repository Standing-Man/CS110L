...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let path_str = fs::read_to_string(format!("/proc/{pid}/fdinfo/{fd}")).ok()?;
        let cursor = OpenFile::parse_cursor(&path_str.as_str())?;
        let access_mode = OpenFile::parse_access_mode(&path_str.as_str())?;
